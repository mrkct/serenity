...
ErrorOr<NonnullRefPtr<SDHostController>> SDHostController::try_initialize()
{
#if ARCH(AARCH64)
    auto hardware_relative_controller_id = StorageManagement::generate_relative_sd_controller_id({});
    auto controller = TRY(adopt_nonnull_ref_or_enomem(new RPi::SDHostController(hardware_relative_controller_id)));
...
{
    u32 scr_bytes[2];
    TRY(issue_command(SD::CommandIndex::AppCmd, relative_card_address));
    TRY(wait_for_response());
    TRY(transaction_control_with_data_transfer_using_the_dat_line_without_dma(
        SD::CommandIndex::AppSendScr,
        0, 1, 8,
        UserOrKernelBuffer::for_kernel_buffer(reinterpret_cast<u8*>(scr_bytes)), DataTransferType::Read));

    return SD::SDConfigurationRegister::from_u64(static_cast<u64>(scr_bytes[1]) << 32 | scr_bytes[0]);
}
