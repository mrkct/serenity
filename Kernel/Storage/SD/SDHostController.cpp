...
bool SDHostController::command_requires_dat_line(SD::EmmcCommand command) const
{
...
ErrorOr<NonnullLockRefPtr<SDHostController>> SDHostController::try_initialize()
{
    ...
    auto controller = TRY(adopt_nonnull_lock_ref_or_enomem(new RPi::SDHostController(hardware_relative_controller_id)));
...
ErrorOr<NonnullLockRefPtr<SDMemoryCard>> SDHostController::try_initialize_inserted_card()
{
    ...
    if (!retry_with_timeout([&]() {
            ...
            if (issue_command(SD::CommandIndex::AppSendOpCond, ACMD41_ARG).is_error())
                return false;
            ...
            if (auto acmd41_response = wait_for_response(); !acmd41_response.is_error()) {
                ...
                ocr = SD::OperatingConditionRegister::from_acmd41_response(acmd41_response.value().response[0]);
            }
    ...
    auto cid = SD::CardIdentificationRegister::from_cid_response(all_send_cid_response.response);
    ...
    TRY(wait_for_response());
    TRY(sync_data_read_command(SD::CommandIndex::AppSendScr, 0, 1, 8, (u8*)scr_bytes));
    auto scr = SD::SDConfigurationRegister::from_u64(static_cast<u64>(scr_bytes[1]) << 32 | scr_bytes[0]);

    TRY(issue_command(SD::CommandIndex::AppCmd, rca));
    TRY(wait_for_response());
    TRY(issue_command(SD::CommandIndex::AppSetBusWidth, 0x2)); // 0b00=1 bit bus, 0b10=4 bit bus
    TRY(wait_for_response());

    // TODO: Ensure the card has the block_size set to 512. Newer cards always assume 512, but olders are more flexible
    // TODO: Change the clock frequency to a higher value
    // TODO: Actually read the card size
    u64 card_capacity_in_blocks = (u64)2 * 1024 * 1024 * 1024 / 512;

    dbgln("SD card found: LUNAddress is {}-{}-{}", controller_id(), 0, 0);
    return TRY(adopt_nonnull_lock_ref_or_enomem(new SDMemoryCard(
        *this,
        // FIXME: Unsure if these 2 params are correct
        StorageDevice::LUNAddress { controller_id(), 0, 0 },
        hardware_relative_controller_id(),
        card_capacity_in_blocks,
        rca,
        ocr,
        cid,
        scr)));
...
ErrorOr<void> SDHostController::issue_command(SD::CommandIndex index, u32 argument)
{
...
ErrorOr<void> SDHostController::sync_data_read_command(SD::CommandIndex index, u32 argument, u32 block_count, u32 block_size, u8* out)
{
