...
u32 SDHostController::calculate_sd_clock_divisor(u32 sd_clock_frequency, u32 frequency)
{
    ...
    if (host_version() == SD::HostVersion::Version2 || host_version() == SD::HostVersion::Version1) {
        ...
        PANIC("SDHostController: Could not find a suitable divisor for the requested frequency");
    }
    ...
    // +------+-------------------------------+
    //
    // NOTE: If "FinalFrequency = BaseClockFrequency / (2 * (Divisor + 1))"
    //       then "Divisor = (BaseClockFrequency / (2 * FinalFrequency)) - 1"
    if (host_version() == SD::HostVersion::Version3)
        return AK::max(AK::ceil_div(sd_clock_frequency, frequency), 2);
...
ErrorOr<void> SDHostController::sd_clock_supply(u32 frequency)
{
    ...
    u32 divisor = calculate_sd_clock_divisor(sd_clock_frequency, frequency);
