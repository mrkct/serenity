    ...
    , ::SDHostController()
{
    ...
    if (auto maybe_registers = Memory::map_typed_writable<SD::HostControlRegisterMap volatile>(physical_address_of_sdhc_registers); !maybe_registers.is_error())
        m_registers = maybe_registers.release_value();
}
